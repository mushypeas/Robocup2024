...
class StoringGroceries:
    ...
    def __init__(self, agent: Agent):
        ...
        # self.available_categories = ['fruit', 'food', 'dish']
        ...
        self.shelf_head_angle = np.arctan(
            ((self.shelf_0_1_height_threshold + self.shelf_1_2_height_threshold) / 2 - 1.0) / 0.6 # 1.0: HSR height, 0.6: distance to shelf
        )
        ...
        self.default_place_dist = 0.1
        self.new_place_dist = self.shelf_width / 2
        self.dist_to_table = 1.0
    ...
    def search_shelf(self):
        dist_to_shelf = distancing(self.agent.yolo_module.pc, self.place_shelf, raw=True)
        center_list = self.agent.yolo_module.detect_3d('grocery_shelf_1f', dist=dist_to_shelf, shelf_depth=self.shelf_depth)
        ...
        for shelf_item_cent_x, shelf_item_cent_y, shelf_item_cent_z, shelf_item_class_id in center_list:
            ...
            rospy.loginfo(f"[{shelf_item_type}] {shelf_item_name} is detected at {shelf_item_floor+1}F")
        ...
        for shelf_item_cent_x, shelf_item_cent_y, shelf_item_cent_z, shelf_item_class_id in center_list:
            ...
            if new_category_floor == 0:
                if shelf_item_cent_z < self.shelf_1_2_height_threshold:
                    shelf_item_dict['new'] = [shelf_item_cent_x, shelf_item_cent_y, shelf_item_cent_z]
            # 2F
    ...
    def pick_item(self, grasping_type, item_name, table_base_xyz):
        ...
        # front
        if grasping_type == 0:
            self.agent.pose.pick_side_pose_by_height(height=self.table_height * 0.94)
            ...
            self.agent.move_rel(table_base_xyz[0] + 0.02, 0, wait=True)
            self.agent.grasp()
            self.agent.pose.pick_side_pose_by_height(height=self.table_height + 0.6)

        # top
        elif grasping_type == 1:
            self.agent.pose.pick_top_pose_by_height(height=self.table_height)
            self.agent.open_gripper()
            table_base_xyz = [axis + bias for axis, bias in zip(table_base_xyz, self.pick_front_bias)]
            self.agent.move_rel(table_base_xyz[0], table_base_xyz[1], wait=True)
            self.agent.pose.pick_top_pose_by_height(height=self.table_height - 0.6)
            self.agent.grasp()
    ...
    def place_item(self, shelf_item_dict, table_item_name, table_item_type):
        ...
        # Place item in shelf
        if shelf_item_cent_z > self.shelf_2_3_height_threshold:
            self.agent.pose.place_shelf_pose('grocery_shelf_3f')
        elif shelf_item_cent_z > self.shelf_1_2_height_threshold:
            self.agent.pose.place_shelf_pose('grocery_shelf_2f')
        else:
            self.agent.pose.place_side_pose('grocery_shelf_1f')

        if shelf_item_cent_y > 0:   # 3d horizontal cent point.
            shelf_base_to_object_xyz = [self.gripper_to_shelf_dist, shelf_item_cent_y - place_object_offset_dist, 0]
        else:
            shelf_base_to_object_xyz = [self.gripper_to_shelf_dist, shelf_item_cent_y + place_object_offset_dist, 0]
        ...
        # Place item at 1F
        if shelf_item_cent_z < self.shelf_1_2_height_threshold:
            self.agent.move_rel(shelf_base_xyz[0] + 0.05, shelf_base_xyz[1], wait=True)  # 2f pose - 1f pose = 0.18
    ...
    def run(self):
        ...
        while True:
            ...
            while not has_grasped:
                ...
                dist_to_table = distancing(self.agent.yolo_module.pc, self.pick_table, dist=self.dist_to_table)
                self.agent.move_rel(dist_to_table, 0)
                ...
                rospy.sleep(0.5)
                table_item_list = np.array(self.agent.yolo_module.detect_3d('grocery_table', dist=self.dist_to_table))
                if table_item_list.size == 0:    # if array is empty(nothing detected)
                    continue
                ...
                for table_item_id in table_item_list_sorted:
                    ...
                    if self.grasp_failure_count[table_item_id] <= 2 and\
                        table_item_type in self.available_categories and\
            ...
            self.agent.move_abs_safe(self.place_location)
            dist_to_table = distancing(self.agent.yolo_module.pc, self.place_shelf, dist=self.gripper_to_shelf_dist)
            # shelf_y = distancing_horizontal(self.agent.yolo_module.pc, place_table)
            self.agent.move_rel(dist_to_table, 0)
