...
def serve_breakfast(agent: Agent):
    ...
    # Item List
    item_list = ['spam', 'mustard', 'spoon'] # 대회 당일에는 item_list 가 인지할 수 있는 것들을 가능한 한 추가 ex)'cracker', 'scrub', 'fork', 'knife' ...

    bowl_to_spam_spill_xy = [0.47, 0.31]  # bowl to cereal
    bowl_to_mustard_spill_xy = [-0.2, 0.1]  # bowl to milk
    ...
    # agent.move_abs('breakfast_table') #bjkim
    ...
    while True:
        ...
        if item == 'bowl':
            ...
            agent.head_text_pub = rospy.Publisher('/Bowl', String, queue_size=4)
            ...
            agent.head_text_pub = rospy.Publisher('/Spam', String, queue_size=4)
            agent.open_gripper()
            agent.move_rel(0, base_xyz[1], wait=True)
            agent.move_rel(base_xyz[0] + 0.3, 0, wait=True)
            agent.grasp()
            rospy.sleep(0.5)
            ...
            agent.head_text_pub = rospy.Publisher('/Mustard', String, queue_size=7)
            agent.open_gripper()
            agent.move_rel(-0.02, base_xyz[1], wait=True)
            ...
            if item == 'spoon':
                ...
                agent.head_text_pub =rospy.Publisher('/Spoon', String, queue_size=5)
                agent.pose.pick_top_pose_last(table=pick_table)
                # agent.pose.pick_top_pose_last(table='breakfast_table') // spoon은 pick_top_pose 사용?
                # agent.pose.pick_bowl_pose_last(table=pick_table)
                agent.open_gripper()
                agent.move_rel(0, base_xyz[1], wait=True)
                agent.move_rel(base_xyz[0] + 0.15 + 0.1, 0, wait=True)
                agent.pose.arm_lift_up(0.495)
                agent.grasp()
                rospy.sleep(0.5)
                agent.pose.arm_lift_up(0.6)
                rospy.sleep(3)
        ...
        if item == 'spam': # 'cereal_red' # elif로 바꿔주기
            import pdb; pdb.set_trace()
            while 1:
                if 'spam' :
                    # 아래 코드는 걍 잠깐 넣은거 실험끝나면 위에 붙여넣어라, 여서부터
