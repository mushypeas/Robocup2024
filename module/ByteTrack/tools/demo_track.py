...
def make_parser():
    ...
    parser.add_argument("--fps", default=120, type=int, help="frame rate (fps)")
...
class Predictor(object):
    ...
    def inference(self, img, timer):
        ...
        img, ratio = preproc(small_img, self.test_size, self.rgb_means, self.std)
...
def image_ros_demo(ros_img, predictor, exp, args, frame_id, tracker, human_id, human_feature, feature_on):
    ...
    if outputs[0] is not None:
        ...
        timer.toc()
        online_im = plot_tracking(img_info['raw_img'], online_tlwhs, online_ids, frame_id=frame_id, fps=30. / timer.average_time)
...
class Bytetrack_ros:
    ...
    def _bytetrack_cb(self, data):
        ...
        if message == 'off_feature':
            print('Stop Feature extraction')
            self.feature_on = False

    def _rgb_cb(self, data):
        self.rgb_img = self.bridge.imgmsg_to_cv2(data, 'bgr8')
        print("size: ", self.rgb_img.shape)
        try:
