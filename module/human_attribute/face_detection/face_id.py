...
class FaceId:
    def __init__(self):
        ...
        self.database_df = pd.read_csv('./module/human_attribute/face_detection/data.csv')
    ...
    def get_all_database_tensor(self, df):
        ...
        full_tensor = None
        for idx in df['id']:
            image_name = './module/human_attribute/face_detection/user_face_database/' + str(idx) + '.jpg'
    ...
    def check_host(self, host_img, seated_face_imgs):
        object_vector = self.get_vector(im2=host_img)

        guest_face_vectors = []
        for face in seated_face_imgs:
