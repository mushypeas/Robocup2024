...
def _get_eyes_midpoint(left_eye_centre, right_eye_centre, image_size):
    ...
    return np.ndarray.astype(np.rint(np.array([
        ...
        (left_eye_centre[1] + right_eye_centre[1]) * image_size[0] / 2])), np.int64)
...
def _rotate_landmarks(landmarks, rotation_matrix, image_size):
    ...
    return np.ndarray.astype(np.rint(np.matmul(rotation_matrix, np.array([
            ...
            np.ones(len(landmarks))]))), np.int64)
...
class FaceCropper:
    ...
    Implements the following pipeline for cropping out faces from an image:
        1. Retrieves bounding boxes and approximate eye coordinates for faces in the image, using the mp.solutions.face_detection.face_detection network
            - For more about this network, visit https://solutions.mediapipe.dev/face_detection
        2. For faces with in-plane rotation (i.e. roll), the face_detection network gives bounding boxes that are much smaller than the face, hence:
    ...
    - The eye coordinates from the mp.solutions.face_detection.face_detection network in step 1 are not used to correct the roll as they are not
      accurate enough and are only good enough for getting an approximate roll angle
    - mp.solutions.face_mesh.FaceMesh has an integrated mp.solutions.face_detection.face_detection model. However, since the integrated network is configured to only
      ...
      separate mp.solutions.face_detection.face_detection model makes the pipeline more configurable to suit requirements
    - mp.solutions.face_mesh.FaceMesh requires a maximum number of faces (max_num_faces) to be specified during initialisation, while mp.solutions.face_detection.face_detection
      does not have such limitation. Hence, Step 1 uses mp.solutions.face_detection.face_detection to get all potential faces, which are then verified with mp.solutions.face_mesh.FaceMesh
      (max_num_faces=1). I.e., if mp.solutions.face_mesh.FaceMesh can detect landmarks in a potential face detected by mp.solutions.face_detection.face_detection, it is taken further in the pipeline
    """
    ...
    def __init__(self, min_face_detector_confidence=0.5, face_detector_model_selection=LONG_RANGE,
        ...
        From mp.solutions.face_detection.face_detection documentation:
    ...
    def get_faces(self, image, remove_background=False, correct_roll=True):
        ...
        for face in detected_faces:
            # The mp.solutions.face_detection.face_detection network may rarely 'find' a face completely outside the image, so ignore those
    ...
    def get_faces_locations(self, image, remove_background=False, correct_roll=True):
        """
        Crop out (and optionally correct the roll and/or remove background of) each detected face in the specified image and return them in a list.
        :param image: A numpy.ndarray RGB image containing faces to be cropped
        :param remove_background: Whether non-face (i.e. background) pixels should be set to 0. Defaults to False
    ...
    def get_faces_debug(self, image, remove_background=False, correct_roll=True):
        ...
        Identical to get_faces(), except it displays the following debug information:
            - Annotations of face detection boxes and eye coordinates from mp.solutions.face_detection.face_detection, approximate roll angle,
